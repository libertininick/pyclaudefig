...
1. Looking up the doc ID using the `fetch-docs` skill
2. Finding official documentation links
3. Updating the frameworks skill with the new entry

## Workflow

### Step 1: Resolve Doc ID

Use the `fetch-docs` skill's "Resolve Library ID" operation to find the doc ID for the framework.

Present the results to the user and confirm the correct doc ID.
...
```markdown
| <Framework> | <Purpose> | `<Doc ID>` | [docs](<URL>) |
```
