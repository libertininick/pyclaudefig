...
The core loop is: **(brainstorm →) plan → implement → review → commit → update plan**, one phase at a time.

Here's what a typical session looks like:

```sh
# 0. (Optional) Brainstorm — when scope is unclear or you want to
#    explore alternatives before committing to an approach.
/brainstorm How should we handle retry logic in the API client?

#    Output: .claude/agent-outputs/brainstorms/<timestamp>-api-client-brainstorm.md
#    The brainstormer interviews you, then produces a summary with
#    a ready-to-use plan prompt. Copy it into /plan.
...
│  /brainstorm  /plan  /implement  /review  /pr-description   │
└─────────────────────────┬───────────────────────────────────┘
                          │ invoke
                          ▼
┌─────────────────────────────────────────────────────────────┐
...
│ brainstormer  planner  code-writer  test-writer  reviewers  │
└─────────────────────────┬───────────────────────────────────┘
                          │ load
                          ▼
┌─────────────────────────────────────────────────────────────┐
